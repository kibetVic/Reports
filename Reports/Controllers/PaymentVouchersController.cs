      ...
      "Eleven", "Twelve", "Thirteen", "Fourteen", "Fifteen", "Sixteen", "Seventeen", "Eighteen", "Nineteen" };
        ...
        public IActionResult CreateWithFiles()
        {
            var model = new PaymentVoucherWithFilesViewModel
            {
                ...
                {
                    VoucheName = VoucherNumber,
                    ChequeNo = "B2C"
                },
                Items = new List<PaymentVoucherItem> { new PaymentVoucherItem() },
        ...
        [RequestFormLimits(MultipartBodyLengthLimit = 100_000_000)] // 100MB limit
        public async Task<IActionResult> CreateWithFiles(PaymentVoucherWithFilesViewModel model)
        {
            ...
            try
            {
                ...
                // Ensure voucher has proper values
                if (string.IsNullOrEmpty(voucher.VoucherNumber))
                {
                    voucher.VoucherNumber = $"VOUCHER-{DateTime.Now:yyyyMMdd-HHmmss}";
                }
                ...
                var itemDescriptions = model.Items?
                ...
                // --- Handle Items ---
                if (model.Items != null && model.Items.Any(i => i != null))
                {
                    voucher.Items = new List<PaymentVoucherItem>();

                    foreach (var item in model.Items.Where(i => i != null && !string.IsNullOrWhiteSpace(i.Description)))
                    {
                        // Calculate amount using the formula
                        item.Amount = (item.EachAmount + item.MpesaCharges) * item.ItemNo;
                        item.PaymentVoucherId = 0; // Will be set after voucher is saved
                ...
                if (model.Files != null && model.Files.Any(f => f != null && f.Length > 0))
                {
                    ...
                    for (int i = 0; i < model.Files.Count; i++)
                    {
                        ...
                        try
                        {
                            ...
                            var safeFileName = $"{batchId}_{Guid.NewGuid()}{Path.GetExtension(originalFileName)}";
                            ...
                            // Get InFavourOf
                            string inFavourOf = "";
                            if (model.InfavourOf != null && i < model.InfavourOf.Count && !string.IsNullOrWhiteSpace(model.InfavourOf[i]))
                            {
                                inFavourOf = model.InfavourOf[i].Trim();
                            }
                            ...
                            var uploaded = new UploadedFile
                            {
                                ...
                                FileType = GetFileType(file.ContentType, file.FileName),
                        ...
                        catch (Exception fileEx)
                        {
                            ...
                            ModelState.AddModelError("", $"Error uploading file {file.FileName}: {fileEx.Message}");
                        }
                    }
                ...
                TempData["Message"] = "Voucher created successfully!";
                return RedirectToAction(nameof(Index));
            }
            catch (Exception ex)
            {
                ModelState.AddModelError("", "Error creating voucher: " + ex.GetBaseException().Message);
                TempData["Error"] = "Error creating voucher: " + ex.GetBaseException().Message;

                // Return to view with model to preserve entered data
                return View(model);
            }
        }
        ...
        private string GetFileType(string? contentType, string fileName)
        {
            ...
            return contentType.StartsWith("image/", StringComparison.OrdinalIgnoreCase) ? "Image" : "Document";
        }
            ...
            try
            {
                ...
                if (replaceFiles != null && model.ExistingIds != null)
                {
                    for (int i = 0; i < model.ExistingIds.Count; i++)
                    {
                        ...
                        // Delete old file
                        var oldPath = Path.Combine(_env.WebRootPath, file.FilePath.TrimStart('/'));
                        if (System.IO.File.Exists(oldPath))
                            System.IO.File.Delete(oldPath);

                        // Upload new file
                        var newFileName = $"{Guid.NewGuid()}{Path.GetExtension(replacement.FileName)}";
                        var newPath = Path.Combine(_uploadPath, newFileName);

                        using (var stream = new FileStream(newPath, FileMode.Create))
                            await replacement.CopyToAsync(stream);

                        file.FilePath = "/uploads/" + newFileName;
                        file.FileType = GetFileType(replacement.ContentType, replacement.FileName);
                ...
                {
                    var filesToDelete = _context.UploadedFiles
                    ...
                    _context.UploadedFiles.RemoveRange(filesToDelete);
                }
                ...
                if (newFiles != null && newFiles.Any())
                {
                    ...
                    for (int i = 0; i < newFiles.Length; i++)
                    {
                        ...
                        catch (Exception fileEx)
                        {
                            ...
                            Console.WriteLine($"Error uploading new file {file.FileName}: {fileEx.Message}");
                        }
                    }
                }
        ...
        public async Task<IActionResult> GeneratePdf(int id)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                // --- Footer ---
                var footerText = "Amtech Plaza, Forest Line, Off Ngong Road, Matasia Shopping Center,  P. O. Box 79701 – 00200 Nairobi.\n" +
